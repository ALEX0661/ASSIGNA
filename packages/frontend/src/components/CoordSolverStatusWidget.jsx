...
/**
 * Coordinator-side twin of SolverStatusWidget.jsx.
 * Draggable floating pill — shows a coordinator's solve status across all
 * coordinator pages. Click it to get a modal with navigation options and a
 * stop button.
 */
export default function CoordSolverStatusWidget() {
  const navigate = useNavigate()
  const location = useLocation()
  const { status, progress, label, processId, dismissed, reset, setStatus, setDismissed } = useCoordSolverStore()
  ...
  async function handleStop() {
    if (!processId) return
    setStopping(true)
    try { await coordCancelSolve(processId) } catch { /* ignore — poller will catch -2 */ }
    setStopping(false)
    setShowModal(false)
    reset()
    setStatus('idle')
  ...
  function handleGoScheduler() {
    ...
    // CoordSchedulerPage reads status straight from the shared store and
    // auto-advances to the right wizard step on mount — no hash needed.
    navigate('/coordinator/scheduler')
  }

  if (status === 'idle' && !cancelledMsg) return null

  // Hide the pill when:
  // 1. User is on the coordinator scheduler page (they have full wizard UI)
  // 2. Status is idle 
  // 3. Status is complete but user has dismissed it
  const hidePill = location.pathname.startsWith('/coordinator/scheduler') 
    || status === 'idle' 
