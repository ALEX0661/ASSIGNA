...
if (!document.getElementById('fd-shared-style')) {
  ...
  s.textContent = `
    ...
    .fd-toast { display:flex; align-items:center; gap:9px; padding:10px 16px; border-radius:12px; font-family:'Inter',sans-serif; font-size:12.5px; font-weight:500; box-shadow:0 8px 28px rgba(0,0,0,0.18); animation:fdToastIn .22s cubic-bezier(.4,0,.2,1); white-space:nowrap; pointer-events:auto; }
    ...
    input:focus, select:focus { border-color:var(--meadow) !important; outline:none; box-shadow:0 0 0 3px rgba(0,0,0,0.12); }
  `
  ...
  return (
    <div style={{ display:'flex', gap:3, alignItems:'center' }}>
      {[1,2,3,4,5].map(s => {
        ...
        return (
          <svg key={s} width={size} height={size} viewBox="0 0 24 24" fill={on ? 'var(--meadow)' : 'none'} stroke={on ? 'var(--meadow)' : 'var(--border)'} strokeWidth="2" style={{ cursor:onChange ? 'pointer' : 'default' }}
