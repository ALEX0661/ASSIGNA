...
/* ─── Styles ─────────────────────────────────────────────────────────────── */
{
  ...
  s.textContent = `
    ...
    .rm-modal-box { background: var(--surface); border-radius: 16px; width: 100%; max-width: 540px; box-shadow: 0 24px 48px rgba(0,0,0,0.25); overflow: hidden; display: flex; flex-direction: column; max-height: 85vh; }
...
export default function CoordCoursesPage() {
  ...
  async function handleEdit(data) {
    setSaving(true); setError('')
    try {
      await updateCourse(editTarget.courseCode, data.program, data)
      setEditTarget(null)
      load()
