...
function OverviewSection({ quality, onExport }) {
  ...
  const KPIS = [
    ...
    {
      label: 'Auto-assigned rate', value: `${quality.autoAssignPct}%`, sub: `${quality.autoAssigned} of ${quality.majorSessions ?? quality.totalSessions} major subjects`,
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    setLoading(true)
    fetchAll().catch(async (err) => setError(await parseError(err, 'load analytics'))).finally(() => setLoading(false))
  }, []) // eslint-disable-line
  ...
  async function handleSelect(value) {
    ...
    if (!name) {
      setLoading(true)
      fetchAll().catch(async (err) => setError(await parseError(err, 'reload analytics'))).finally(() => setLoading(false))
      return
    }
    setSwitching(true)
    try { await loadSaved(name); await fetchAll() }
    catch (err) { setError(await parseError(err, `load schedule "${name}"`)) }
  ...
  async function handleRefresh() {
    ...
    try {
      ...
      setError(await parseError(err, 'refresh analytics'))
    } finally {
      setRefreshing(false)
    }
  }
